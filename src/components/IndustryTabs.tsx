...
export function IndustryTabs({ industries, activeTab, onTabChange }: IndustryTabsProps) {
  ...
  useEffect(() => {
    const activeIndex = industries.findIndex((industry) => industry.id === activeTab);
  ...
  const handleTouchEnd = () => {
    ...
    if (Math.abs(swipeDistance) > swipeThreshold) {
      const currentIndex = industries.findIndex((industry) => industry.id === activeTab);
      
      if (swipeDistance > 0 && currentIndex < industries.length - 1) {
        // Swipe left - next tab
        onTabChange(industries[currentIndex + 1].id);
